...
public class Water : MonoBehaviour
{
    ...
    public static Water Instance
    {
        ...
        {
            if (sInstance == null)
                sInstance = FindObjectOfType<Water>();
            return sInstance;
        }
    }
    private static Water sInstance;
    private void Awake()
    {
    ...
    void Start()
    {
        waterCamera = new GameObject("WaterCamer").AddComponent<WaterCamera>();
        waterCamera.transform.SetParent(transform);
        waterCamera.transform.localPosition = Vector3.zero;
        waterCamera.transform.localEulerAngles = new Vector3(0, 0, 0);
        waterCamera.Init(100, 100, 10);

        // waterMeshRenderer = gameObject.GetComponent<MeshRenderer>();
        // if (waterMeshRenderer == null) waterMeshRenderer = gameObject.AddComponent<MeshRenderer>();
        // if (waterMeshFilter == null) waterMeshFilter = gameObject.AddComponent<MeshFilter>();
    ...
    public void DrawMesh(Mesh mesh, Matrix4x4 matrix)
    {
        if (Instance != null)
        {
            Instance.waterCamera.ForceDrawMesh(mesh, matrix);
        }
    }
    #region 遍历交点，传入矩阵
    Matrix4x4 matrixR;
    void MakeRangeMatrix()
    {
        ...
        if (crossPoints.Count == 3)
        {
            ...
            waterMaterial.SetMatrix("_RangeMatrix", matrixR);
            return;
        }
        // 0    1
        //
